...
//    }
    ...
    public void findPwd(@PathVariable("email") String email) {
        ...
        String subject = "[Grid] 비밀번호를 변경해주세요"; // 이메일 제목
        String body = "<div style=\"text-align: center; font-family: Arial, sans-serif;\">"
                ...
                + "<a href=\"http://www.gridhr.site/find/pwd/" + encodedEmail + "/result\" style=\"display: inline-block; margin-top: 20px; padding: 10px 20px; font-size: 16px; color: #ffffff; background-color: #5865F2; border-radius: 5px; text-decoration: none;\">Reset Password</a>"
                + "</div>"
                + "</div>"; // 이메일 본문
