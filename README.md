...
결재 유형별로 다른 결재 라인을 결재 라인 테이블에 저장
결재 유형에 따라서 다른 테이블에 결재 데이터 저장
결재 생성과 동시에 결재 라인 규칙에 맞춰서 생성되는 결재 라인 데이터 저장

각 유형마다 결재 라인의 개수가 다르게 생성되고 컬럼에도 차이가 있어 다른 테이블로 나누어 설계
취소 결재는 테이블을 따로 생성하지 않고 취소 문서 id 컬럼과 취소 여부 컬럼으로 관리하는 방향으로 설계
