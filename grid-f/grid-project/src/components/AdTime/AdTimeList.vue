...
const fetchAllAdTime = async () => {
  try {
    const response = await axios.get('https://api.gridhr.site/ad-time/all')
    ...
    const responseTodayAd = await axios.get(`https://api.gridhr.site/ad-time/date/${today}`)
    ...
    //오늘 출장 인원 조회
    const responseTodayBt = await axios.get(`https://api.gridhr.site/approval/today/bt`)
    cntBt.value = responseTodayBt.data.approvalEmpResultList.length;


    //오늘 휴가 인원 조회
    const responseTodayVacation = await axios.get(`https://api.gridhr.site/approval/today/v`)
...
const fetchEmployeeAdTime = async () => {
  try{
    const response = await axios.get(`https://api.gridhr.site/ad-time/${userId.value}`);
