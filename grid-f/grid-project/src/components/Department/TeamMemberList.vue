...
const fetchEmployees = async (teamId) => {
  try {
    ...
    // 모든 직원의 상세 정보를 가져옴
    const employeeDetailsPromises = employeeIds.map(id => axios.get(`https://api.gridhr.site/users/id/${id}`));
...
const getTeamName = async (teamId) => {
  try {
    const response = await axios.get(`https://api.gridhr.site/team/${teamId}`);
